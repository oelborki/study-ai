...
import { useTheme, THEME_COLORS, ThemeColorKey } from "./ThemeContext";
...
export default function Sidebar({ user }: { user: User | null }) {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      const insideDesktopSettings =
        desktopSettingsRef.current?.contains(target) ?? false;
      const insideMobileSettings =
        mobileSettingsRef.current?.contains(target) ?? false;
      if (!insideDesktopProfile && !insideMobileProfile) {
        setProfileOpen(false);
      }
      if (!insideDesktopSettings && !insideMobileSettings) {
  ...
  const renderSettingsPopover = (expanded: boolean) =>
    settingsOpen && (
      <div
        className={`absolute z-50 w-64 rounded-lg bg-[#1a1a24] border border-white/10 shadow-xl py-1 ${
          expanded
      ...
      >
        ...
        <div className="px-4 py-3">
          <p className="text-xs font-medium text-[#94A3B8] mb-3">Theme</p>
              ...
              return (
                <button
                  ...
                  className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                    isSelected
                      ? "ring-2 ring-offset-2 ring-offset-[#1a1a24]"
                      : "hover:scale-110"
                  }`}
                  style={{
                    backgroundColor: color.main,
                    ["--tw-ring-color" as string]: isSelected ? color.main : undefined,
                  }}
          ...
          <button
            ...
            className="mt-4 w-full text-xs text-[#94A3B8] hover:text-white transition-colors py-2 rounded-lg border border-white/10 hover:border-white/20"
          >
            Restore Default
          </button>
        </div>
  ...
  return (
    <>
      ...
      >
        ...
        {/* Settings button */}
        <div className="relative px-3 pb-2" ref={desktopSettingsRef}>
          <button
            onClick={() => setSettingsOpen((prev) => !prev)}
            title={collapsed ? "Settings" : undefined}
            className={`flex items-center gap-3 w-full rounded-lg px-3 py-2.5 text-sm font-medium text-[#94A3B8] hover:bg-white/5 hover:text-white transition-colors ${
              collapsed ? "justify-center" : ""
            }`}
          >
            {settingsIcon}
          ...
          {renderSettingsPopover(!collapsed)}
        </div>
      ...
      >
        ...
        {/* Settings button */}
        <div className="relative px-3 pb-2" ref={mobileSettingsRef}>
          <button
            onClick={() => setSettingsOpen((prev) => !prev)}
            className="flex items-center gap-3 w-full rounded-lg px-3 py-2.5 text-sm font-medium text-[#94A3B8] hover:bg-white/5 hover:text-white transition-colors"
          >
